...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verificação removida - estar nesta página já indica que é administrador

    const { userId, serviceIds, barberId } = await request.json();
    ...
    if (existingQueue) {
      return NextResponse.json({ error: "Este usuário já está na fila" }, { status: 400 });
    }
